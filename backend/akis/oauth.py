...
from .models import OAuthApp,OAuthState,Credential,now
from .security import encrypt,decrypt,valid_session
...
def begin(db,platform,owner,session,account_id=''):
    app=db.get(OAuthApp,platform)
    ...
    db.add(OAuthState(id=state,owner=owner,session_hash=hashlib.sha256(session.encode()).hexdigest(),platform=platform,verifier=encrypt(json.dumps({'verifier':verifier,'account_id':account_id,'redirect_uri':app.redirect_uri})),expires_at=now()+600))
    db.commit()
...
def callback(platform:str,req:Request):
    ...
    try:
        ...
        if not valid_session(session): raise HTTPException(401,'Oturumun süresi doldu. Akış’a yeniden giriş yap.')
        with Session() as db:
            row=db.get(OAuthState,state)
            if not row or row.platform!=platform or row.expires_at<now() or row.used or row.session_hash!=hashlib.sha256(session.encode()).hexdigest(): raise HTTPException(400,'Bağlantı isteğinin süresi dolmuş veya daha önce kullanılmış.')
            ...
            app=db.get(OAuthApp,platform);details=json.loads(decrypt(row.verifier))
            ...
            c=db.scalar(select(Credential).where(Credential.owner==row.owner,Credential.platform==platform))
            if not c: c=Credential(owner=row.owner,platform=platform,access_token='');db.add(c)
            ...
            c.updated_at=now();c.refresh_error=None;c.token_kind=kind;c.scopes=data.get('scope',SCOPES[platform]);db.commit()
